// Telephone Words
// Each number key on a standard phone keypad has a set of Latin letters written on it as well: http://en.wikipedia.org/wiki/File:Telephone-keypad2.svg

// Businesses often try to come up with clever ways to spell out their phone number in advertisements to make it more memorable.
// But there are a lot of combinations!

// Write a function that takes up to four digits of a phone number, and returns a list of all of the words that can
// be written on the phone with that number. (You should return all permutations, not only English words.)

// INPUT: "0002"
// OUTPUT: [ "000A", "000B", "000C" ]

// INPUT: "1123"
// OUTPUT: [ "11AD", "11AE", "11AF", "11BD", "11BE", "11BF", "11CD", "11CE", "11CF" ]

function telephoneWords (digitString) {
  var phoneNum = {
    0:[0],
    1:[1],
    2:['A','B','C'],
    3:['D','E','F'],
    4:['G','H','I'],
    5:['J','K','L'],
    6:['M','N','O'],
    7:['P','Q','R','S'],
    8:['T','U','V'],
    9:['W','X','Y','Z']
  }
  var combo = [];
  // create an array to hold results
  var results = [];

  function recurse (position){
    // base case:
    // if position === combo.length
    if(position === digitString.length){
      // join combination, push to results
      results.push(combo.join(''));
      // return;
      return;
    }
    // recurse case:
    // iterate through string
    var digit = phoneNum[digitString.charAt(position)];
    for(var i = 0; i < digit.length; i++){
      // push letter to combination
      combo.push(digit[i]);
      // recurse(position+1)
      recurse(position + 1);
      // combination.pop()
      combo.pop();
      }
    }
  // recurse at zero
  recurse(0);
  // return results
  return results;
}
